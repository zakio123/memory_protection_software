...
static inline uint64_t compute_data_tag(uint8_t cipher64[64], uint64_t counter, uint64_t dram_addr,const int minor_bit_width){
...
struct CtrDeriver {
  ...
  aes::Block make_ctr_block(uint64_t block_addr, uint64_t major, int sub) const {
    aes::Block iv{};
...
{
  for(int k=0;k<4;++k){
    aes::Block ctr = der.make_ctr_block(block_addr, major, k);
...
static void encrypt64_AES_CTR_tls(const aes::AES128& aes_tls,
                                  ...
                                  uint64_t block_addr, uint64_t major,
                                  const CtrDeriver& der = {})
{
  for(int k=0;k<4;++k){
    aes::Block ctr = der.make_ctr_block(block_addr, major, k);
...
//   一番最初だけ親ノードが64bitde1
  ...
  int height = HEIGHT;
  const std::string out_path = "out_mono_counter_image_mcw" + std::to_string(minor_bit_width) + "_mcc" + std::to_string(minor_count) + "_height" + std::to_string(height) + ".img";
  {
