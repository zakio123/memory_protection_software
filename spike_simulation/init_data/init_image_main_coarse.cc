...
#include "aes128.h"
#include "config.h"
static constexpr uint64_t BLOCK_SIZE = PROTECTION_SIZE_GRAIN;
...
static inline uint64_t compute_data_tag(uint8_t* cipher64, uint16_t counter_byte, uint64_t dram_addr,const int minor_bit_width){
...
// 暗号文を入れるvector
  ...
  // データタグ領域（BLOCK_SIZEで1タグ 8*8B）
  const uint64_t n_tag_blocks = div_ceil_u64(n_blocks / (BLOCK_SIZE / 64), 8);
  ...
  #pragma omp parallel for schedule(static)
    for(uint64_t b=0;b<n_blocks / (BLOCK_SIZE / 64);++b){
        uint8_t* ciph = &cipher[b*BLOCK_SIZE];
        uint64_t tag = compute_data_tag(ciph, init_minor, PROTECTION_BASE + b * BLOCK_SIZE, SETTING_MINOR_COUNTER_WIDTH);
  ...
  for(uint64_t n=0;n<total_nodes;++n){
      ...
      //     set_minor_val(node, k, 0, MINOR_COUNTER_WIDTH); 
      // }
  ...
  int height = HEIGHT;
  const std::string out_path = "out_image_mcw" + std::to_string(minor_bit_width) + "_mcc" + std::to_string(minor_count) + "_height" + std::to_string(height) + "_coarse" + ".img";
  {
