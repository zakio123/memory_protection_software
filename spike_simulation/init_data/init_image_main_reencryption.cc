...
// 暗号文を入れるvector
  ...
  // 暗号化
    ...
    const uint16_t init_minor = (1 << MINOR_COUNTER_WIDTH) - 1; // 最大値で初期化
    CtrDeriver der;
  #pragma omp parallel
  {
...
//   mac = 8B
//   一番最初だけ親ノードが64bitde1
  uint64_t parent_counter = (1 << MINOR_COUNTER_WIDTH) - 1; // 初期値
  ...
  for(uint64_t n=0;n<total_nodes;++n){
      ...
      for (int k = 0; k < MINOR_COUNTER_COUNT; ++k) {
          ...
          set_minor_val(node, k, init_minor, MINOR_COUNTER_WIDTH); 
      }
      uint64_t mac = compute_node_mac(node, parent_counter, (n==0)?64:SETTING_MINOR_COUNTER_WIDTH, dram_addr);
      if (n == 0) {
          std::cout << "[Debug] Root node MAC: " << std::hex << mac << std::dec << std::endl;
      }
  ...
  int height = HEIGHT;
  const std::string out_path = "out_image_reencryption_mcw" + std::to_string(minor_bit_width) + "_mcc" + std::to_string(minor_count) + "_height" + std::to_string(height) + ".img";
  {
