...
typedef enum {
    ...
    RS_AES_XOR_START,     // xor_start / copy_xor 設定
    ...
    RS_RESP,    // spm_write_back
    RS_DONE,              // 完了
    RS_ERROR,
    RS_EVICT_NODE         // ノードのイジェクト処理
} req_state_t;
...
void step(AddressContext *context, index_t i, dma_id_t *global_dma_id) {
    // 状態に応じた処理を行う
    // if (context->state != RS_IDLE && context->state != RS_DATA_WAIT){
    //     printf("[Core FW] Processing state %d for addr %016llx\n", context->state, context->request_addr);
    // }
    if (context->state == RS_DATA_WAIT) {
        if (dma_wait(context->internal_wait_id)){
            if (context->is_write){
                context->state = RS_AES_XOR_WAIT;
            } else {
                context->state = RS_DATAMAC_UPDATE;
            }
            context->internal_wait_id += 1;
        }
    } else if (context->state == RS_DATAMAC_WAIT) {
        if (context->tag_hit == false){
            if (dma_wait(context->internal_wait_id)){
                context->state = RS_DATAMAC_VERIFY_AUTHENTICATE;
                context->internal_wait_id += 1;
            }
        } else {
            context->state = RS_DATAMAC_VERIFY_AUTHENTICATE;
  ...
  switch (context->state) {
    ...
    case RS_PATH_TAG_WALK:{
        ...
        for (uint64_t i = context->load_start_index;i<HEIGHT;i++){
            ...
            context->temp_index[i] = alloc_temp_entry(context->path_dram_addrs[i], context->path_spm_offsets[i]);
            dram_addr_t dram_addr = context->path_dram_addrs[i];
            spm_copy_to_local(dram_addr, context->path_spm_offsets[i], 64, *global_dma_id);
        }
        // データブロック
        dram_addr_t data_addr = context->request_addr;
        (*global_dma_id)++;
        spm_copy_to_local(data_addr, DATA_SPM_OFFSET + 64 * i, 64, (*global_dma_id));
        dram_addr_t mac_addr = get_datamacblock_addr(context->request_addr);
        ...
        (*global_dma_id)++;
        spm_copy_to_local(mac_addr, context->mac_spm_offset, 64, (*global_dma_id));
        context->state = RS_PATH_VERIFY;
        // printf("dma issued up to id %llu\n", *global_dma_id);
        // exit(1);
        break;
    }
    case RS_PATH_VERIFY:{
    ...
    case RS_SWAP_OR_EVICT:{
        ...
        dma_id_t temp_id = *global_dma_id;
        for (uint64_t i = context->load_start_index;i<HEIGHT;i++){
        ...
        *global_dma_id = temp_id;
        context->state = RS_SET_SEED;
        break;
    }
    case RS_SET_SEED:{
        // major/minor counterから set_seed
    ...
    case RS_AES_XOR_START:{
        // xor_start / copy_xor 設定
        if (context->is_write){
            // write_xor(DATA_SPM_OFFSET + 64 * i);
            xor_start(true, false);
            copy_xor(DATA_SPM_OFFSET + 64 * i);
            context->state = RS_DATAMAC_UPDATE;
        } else {
            write_xor(DATA_SPM_OFFSET + 64 * i);
    ...
    case RS_AES_XOR_WAIT:{
        if (!AES_START_REG){
            ...
            // context->state = RS_AES_XOR_START;
        }
        break;
    }
...
int main(void){
  ...
  while(1){
    // リクエスト待ち
    for(;;){
      if (AXIM_STATUS_REG != 0 && active_processes < 1){
        // あればリクエスト登録
        int slot = free_slots[free_slot_top--];
        if (AXIM_STATUS_REG & 2){ // writeリクエスト
            // printf("[Core FW] AXIM Write Request received\n");
            process_list[slot].is_write = true;
        } else {
            // printf("[Core FW] AXIM Read Request received\n");
        ...
        // printf("[Core FW] New request received: addr=%016llx, is_write=%d, req_id=%u\n", addr, process_list[slot].is_write, req_id);
      } else {
        break;
      }
    }
    for (int i=0;i<1;i++){
      if (process_list[i].state == RS_DONE){
        // printf("[Core FW] Request completed: addr=%016llx, is_write=%d, req_id=%u\n", process_list[i].request_addr, process_list[i].is_write, process_list[i].req_id);
