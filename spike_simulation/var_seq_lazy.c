...
static inline void swapp_dram_addr(dram_addr_t dram_addr,bool is_leaf){
  ...
  long idx = find_temp_entry(dram_addr);
  if (idx == -1){
    printf("Error: no temp entry found for swapping: addr=%016llx\n", dram_addr);
    return;
  }
  spm_offset_t temp_spm = get_temp_spm_offset(idx);
...
void Authentication(dram_addr_t request_addr, uint32_t req_id){
  ...
  if (current_minor_val == MINOR_COUNTER_MASK) {
      ...
      // if (request_addr == 0x91adde40){
      //   for (int j = 0;j<8;j++){
      //     uint64_t val = spm_ld64(spm_offset_array[0] + j * 8);
      //     printf(" after reencryption lazy spm[%d]: %016llx\n", j, val);
      //   }
      // }
      major_counter += 1;
  ...
  if (request_addr == 0x91adde40){
    printf("authen addr %016llx major %llu minor %u\n", request_addr, major_counter, minor_counter_value);
  }
...
void Verification(dram_addr_t request_addr, uint64_t req_id){
  ...
  for (uint64_t i = 0;i<hit_index;i++){
    ...
    dma_id_t need_id = (i == HEIGHT - 1) ? wait_dma_id[HEIGHT - 1] : wait_dma_id[i+1];
    mac_req_id = global_mac_req_id;
    verify_one_height_lazy(spm_offset_array[i], parent_spm, v_i, mac_req_id,need_id, dram_addr_array[i]);
    global_mac_req_id += 1;
  ...
  // 結果の使用
  if (request_addr == 0x91adde40){
    printf("Verification addr %016llx major %llu minor %u\n", request_addr, major_counter, minor_counter_value);
  }
  set_seed(major_counter, minor_counter_value, request_addr);
