...
#define CACHE_WAYS (4)
#define CACHE_SETS (24) // 24line
#define CACHE_DATA_SPM_BASE (64 * 4) // データ領域のベースアドレス
#define SPM_METADATA_BASE (112 * 64)
...
bool tag_check(uint64_t dram_addr){
  ...
  for (uint64_t i = 0; i < CACHE_WAYS; ++i) {
      ...
      bool valid = (info & 1) == 1;
      uint64_t current_block_addr = (info >> 6) << 6;
      if (valid && current_block_addr == block_addr) {
...
bool verifyTreePath(const uint64_t* path_indecis){
  ...
  //   uint64_t dram_addr = COUNTER_BASE + path_indecis[i-1] / 32 * 64 + calculate_level_base_addr(i);
  //   bool valid = tag_check(dram_addr);
  //   // printf("Level %llu: dram_addr=%016llx, tag_valid=%d\n", i-1, dram_addr, valid);
  //   if (valid){
  //     start_level = i;
  //     break;
  //   }
  ...
  // printf("Starting verification from level %llu spm_offset %llu\n", start_level, prev_offset);
...
int main(void){
  ...
  uint64_t nu = 4096 * 32; // 4096リクエス1ト 
  memreq_make(size, 64); // 64B, 1リクエスト
  ...
  while(1){
    ...
    if (addr == 0xFFFFFFFFFFFFFFFF){
      ...
      if(AXIM_STATUS_REG & 2){ // writeリクエスト
        ...
        printf("[Core FW] Read request received for address %016llx\n", addr);
        Verification();
      }
    }
  }
